 ...
 * Handles both the old plain format (comma/semicolon separated) and the new
 * rich format generated by buildCrewContext: "Name [BES, DSA] — notes".
 * Strips flags and notes, returning only the clean name for each student.
 */
export const parseCrewContextToNames = (context: string): string[] => {
    ...
    return context
        .split('\n')
        .flatMap(line => {
            // Strip rich format: "Name [BES, DSA] — BES: ... | Note: ..." → "Name"
            const cleaned = line
                .replace(/\s*\[.*?\]/g, '')  // rimuove [BES, DSA, PEI]
                .replace(/\s*—.*$/s, '')      // rimuove — e tutto ciò che segue
                .trim();
            // Compatibilità col vecchio formato a virgole su una sola riga
            if (cleaned.includes(',')) {
                return cleaned.split(',').map(n => n.trim()).filter(Boolean);
            }
