...
const { students, syncStudentsWithContext, addEvaluationToStudent, recordAttendanceForBlock, addStructuredStudent, updateStructuredStudent, deleteStructuredStudent } = useStudents(masterContext.crewContext);
...
const [notebookToEdit, setNotebookToEdit] = useState<Partial<Notebook> | null>(null);
  ...
  const { modules, contentUnits, parsedMethodologies } = useProgettazioneCache();
...
const getOrCreateConversationForWeek = useCallback((weekInfo: WeekRouteInfo): Conversation => {
  ...
  // --- Handler factories ---
  const {
    handleSetWeekTheme, handleUpdateWeekTheme, handleUpdateBlockObjective, handleUpdateBlockSubject, handleUpdateBlockTitle,
  ...
  const {
    ...
    handleUpdateBlockMetodologia, handleToggleFslPeriod,
    handleToggleExternalExpert, handleUpdateExternalExpertName,
    handleToggleFuoriAula, handleUpdateLuogo,
    handleSaltaChoice,
    handleArchiveDetachedLesson, handleRelocateDetachedLesson,
  ...
  const {
    handleUpdateBlockInConversation, handleReEditBlock, handleAddActivity, handleAddActivityForBlock,
  ...
  const {
    ...
    handleAddLessonMaterial, handleRemoveLessonMaterial, handleSaveMaterialBrief,
    ...
    handleSaveAssignmentPlan, handleUpdateAssignmentChannel,
  ...
  const {
    ...
    updateConversation, handleSelectConversation, setSelectedStudentId,
    setInitialPlanningTab, setNotebookToEdit, setModalState, setView: setViewFn,
  }), [updateConversation, handleSelectConversation, setInitialPlanningTab, setNotebookToEdit, setModalState]);
        ...
        {
          {
            ...
            'lezione': <InAulaView conversations={conversations} onClose={() => setView('lobby')} students={students} onNavigateToBlock={handleNavigateToBlock} onFormatMultipleBlocks={handleFormatBlocks} onRecordAttendance={handleRecordAttendanceForBlock} onSaveGroups={handleSaveGroupsForBlock} onAddArtifact={handleAddArtifactForBlock} onDeleteArtifact={handleDeleteArtifactForBlock} onOpenLessonNotesModal={setLessonNotesModalInfo} onDeleteLessonNotes={handleDeleteLessonNotes} onGenerateAnalysis={handleGenerateAnalysis} analysisLoadingBlockId={analysisLoadingBlockId} onUpdateGroups={handleUpdateGroupsForBlock} onUpdateGroupNotes={handleUpdateGroupNotesForBlock} showToast={showToast} masterContext={masterContext} onUpdateBlockStatus={handleUpdateBlockStatus} onAddLink={handleAddLinkForBlock} onDeleteLink={handleDeleteLinkForBlock} onUpdateCloudLink={handleUpdateBlockCloudLink} notebooks={notebooks} onAddNotebook={addNotebook} onUpdateLinkedNotebooks={handleUpdateBlockLinkedNotebooks} onAvviaLezione={handleAvviaLezione} onChiudiLezione={handleChiudiLezione} onAddMaterial={handleAddLessonMaterial} onRemoveMaterial={handleRemoveLessonMaterial} onSaveMaterialBrief={handleSaveMaterialBrief} onSetAttendance={handleUpdateLiveAttendance} onAddEvaluation={handleAddLessonEvaluation} onRemoveEvaluation={handleRemoveLessonEvaluation} onAutoSaveNotes={handleAutoSaveLessonNotes} onGenerateLessonNoteAnalysis={handleGenerateLessonNoteAnalysis} onSaveClassroomUrl={handleSaveClassroomUrl} activities={allActivities} onUpdateActivity={handleUpdateActivity} onGenerateBriefing={handleGenerateBriefing} onAddObservation={handleAddActivityObservation} onRecordSubmission={handleRecordActivitySubmissionFull} onLaunchActivity={handleLaunchActivity} onSaveAssignmentPlan={handleSaveAssignmentPlan} onUpdateAssignmentChannel={handleUpdateAssignmentChannel} routeWeeks={availableWeeks} onSavePreparationSources={handleSavePreparationSources} />,
            'classroom_trend': <ClassroomTrendView conversations={conversations} students={students} onClose={() => setView('lobby')} onSelectStudent={handleSelectStudent} onOpenGroups={() => setView('groups_archive')} />,
            'groups_archive': <GroupsArchiveView conversations={conversations} students={students} onClose={() => setView('lobby')} masterContext={masterContext} onUpdateBlock={handleUpdateBlockInConversation} activities={allActivities} onSelectStudent={handleSelectStudent} />,
            'student_profile': <StudentProfileView student={selectedStudent!} onClose={() => setView('roster')} onOpenImportModal={handleOpenImportModal} conversations={conversations} />,
