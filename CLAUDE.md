...
components/
  ...
  handlers/                  — handler estratti da MainApp.tsx (split 2026-05-25):
    ...
    blockHandlers_status.ts  — stato blocco: handleUpdateBlockStatus, handleUpdateBlockTipologia, handleUpdateBlockMetodologia, handleToggleFslPeriod, handleToggleExternalExpert, handleUpdateExternalExpertName, handleToggleFuoriAula, handleUpdateLuogo, handleSaltaChoice, handleArchiveDetachedLesson, handleRelocateDetachedLesson (coda contenuti — 2026-07-06) + createApplyBlockStatus (helper)
  ...
  StrategicDashboardView.tsx — "Progettazione del Corso": settimane (da routeCalendar), blocchi, progressStats; accordion blocco con selettori "Cosa" + "Come" + "Approccio" (metodologia) + toggle FSL / ESP / FUORI + badge contestuali nell'header + campi esperto/luogo nella sezione espansa + campo "Obiettivo Didattico" + "Titolo" nell'header. Due pulsanti AI: ✦ → TitleSuggestionModal; "Suggerisci obiettivo" → ObjectiveSuggestionModal. Il radar NON è nell'header (in GanttView). Coda dei contenuti (2026-07-06): il badge ambra "N contenuti in sospeso" è un bottone che apre un pannello inline sotto l'header con i DetachedLesson attivi; azioni per item: "Rimanda" (onRelocateDetached → primo blocco libero) e "Archivia" (onArchiveDetached). Props opzionali onArchiveDetached/onRelocateDetached.
  ...
  GanttView.tsx              — "Analisi del Corso": colonna sinistra = Gantt attività + SubjectHeatmap (Argomento×Tipologia) + ModuloMetodologiaMatrix (Modulo×Metodologia) + ContestoFisicoChart (in aula/fuori aula per modulo, visibile solo se isFuoriAula usato) + MetodologiaContestoChart (incrocio metodologia × fuori aula, sotto ContestoFisicoChart nello stesso pannello — 2026-07-06); colonna destra = ModuleDonut + DidacticRadarChart. Calcola radarData, heatmapRows, matrixRows, contestoRows, metodoContestoRows direttamente dalle conversations.
  ...
  StudentDimensionRadar.tsx  — radar dimensioni operative studente (2026-07-06): componente SEPARATO da DidacticRadarChart (che resta vincolato ai LessonType). 5 assi 0–100: Partecipazione, Puntualità, Completamento (lavori di gruppo), Valutazioni (parseEvaluationScore in StudentProfileView), Segnali Ada. value null = asse spento "n.d.". Colore viola (#a78bfa), badge indice medio.
...
- Non aggiungere textarea di input o pulsanti di generazione AI in `ClassroomTrendView` — la sezione MONITORAGGIO è riservata ai cruscotti di sola lettura. Zero funzioni operative. (2026-05-27) I report qualitativi del monitoraggio (Diario Qualitativo, note gruppi, sentiment) derivano SOLO da analisi Ada già salvate altrove — nessuna nuova chiamata AI dal monitoraggio (confermato 2026-07-06).
- Non leggere `block.lessonGroups` — campo legacy mai scritto. I gruppi vivono in `block.allocations.data.groups` (scritti da handleSaveGroupsForBlock/handleUpdateGroupsForBlock/record_groups). Il fallback `?? block.lessonGroups` in LessonPreparationTab esiste solo per retrocompatibilità DB (2026-07-06).
- Non usare `DidacticRadarChart` per le dimensioni operative dello studente — usare `StudentDimensionRadar` (componente separato, assi 0–100, non LessonType). I due radar non vanno unificati (2026-07-06).
- Non trattare i lavori individuali come entità separata nei report gruppi — un lavoro individuale È un GroupDefinition con 1 studentId (badge "individuale"). Non reintrodurre `individualAssignments` nella UI (2026-07-06).
- Non far slittare i blocchi quando si ricolloca un contenuto dalla coda — `handleRelocateDetachedLesson` scrive sul primo blocco LIBERO (senza objective/titolo/messaggi/contentBlocks), mai su blocchi occupati. Nessuna rinumerazione (2026-07-06).
